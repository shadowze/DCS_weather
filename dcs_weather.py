...
s_version_info = "V3.1_2023/DEC/31_09:40"
...
# dcs_weather.py D:\DCS_Missions\Weapons_Training_v2.11.2.miz UGSB UGSS new
# dcs_weather.py D:\DCS_Missions\Weapons_Training_v2.11.2.miz UGSB UGSS 3
...
# to use full moon days <time control> = new
...
class G:  # Globals class ...... to store all the data we need to mess with (populate with some defaults)
    ...
    s_start_time = ''
    i_rand_template = 1
    # presets used if time is too late in day and dark eg autumn / winter, first block is CA second PG / SY
    l_dates = [['2021', '03', '28', '7', '30'], ['2021', '06', '24', '5', '50'], ['2021', '09', '21', '7', '45'],
               ['2021', '12', '19', '8', '30'], ['2021', '03', '28', '14', '0'], ['2021', '06', '24', '16', '0'],
               ['2021', '09', '21', '14', '0'], ['2021', '12', '19', '13', '0'],

               ['2021', '03', '28', '6', '10'], ['2021', '06', '24', '5', '0'], ['2021', '09', '21', '5', '0'],
               ['2021', '12', '14', '6', '30'], ['2021', '03', '28', '12', '0'], ['2021', '06', '24', '13', '0'],
               ['2021', '09', '21', '12', '0'], ['2020', '12', '19', '12', '0']

               ]

    # presets for 2024, these dates are when the moon is full, server at index zero will be Jan 25th @ 13:00hrs start
...
def get_avwx_weather_cloud_atmosphere(json_weather_l):  # read json and set the cloud settings from it
    ...
    if s_cloud_cover in ('SKC', 'NCD', 'CLR', 'NSC'):
        G.s_cloud_density = '0'  # use preset1 for this
        G.s_cloud_preset = 'Preset1'
    elif s_cloud_cover in 'FEW':
        G.s_cloud_density = str(rd.randint(1, 2))  # use preset4
        G.s_cloud_preset = 'Preset4'
    elif s_cloud_cover in 'SCT':
        G.s_cloud_density = str(rd.randint(3, 4))  # use preset10
        G.s_cloud_preset = 'Preset10'
    elif s_cloud_cover in 'BKN':
        G.s_cloud_density = str(rd.randint(5, 8))  # use preset13
        G.s_cloud_preset = 'Preset13'
    elif s_cloud_cover in 'OVC':
        G.s_cloud_density = '9'  # use preset21
        G.s_cloud_thickness_m = '200'
        G.s_cloud_preset = 'Preset21'
    elif s_cloud_cover in 'VV':
        G.s_cloud_density = str(rd.randint(2, 8))  # use preset21
        G.s_cloud_preset = 'Preset21'
    ...
    if any(x in s_raw_report for x in l_rain):
        #  print("RA found")
        G.s_iprecptns = '1'  # rain  RainyPreset1
        G.s_cloud_preset = 'RainyPreset2'
        # G.s_cloud_preset = 'Preset4'
...
def get_avwx_weather_wind(json_weather_l):  # read json weather data and set the wind info from it
    ...
    if i_winds_size == 0:
        G.s_wind_speed_gnd = str(rd.randint(0, 3))
        G.s_wind_dir_2k = gen_rand_dev(G.s_wind_dir_gnd, -2, 2)
        G.s_wind_dir_8k = gen_rand_dev(G.s_wind_dir_gnd, -3, 3)
    else:
    ...
    G.s_wind_speed_2k = gen_rand_dev_speed(s_mps, -2, 2)

    # wind at 8k - we use ground level value and randomise a bit more
    G.s_wind_speed_8k = gen_rand_dev_speed(s_mps, -3, 3)
...
def get_checkwx_weather_wind(json_weather_l):
    ...
    G.s_wind_dir_2k = gen_rand_dev(G.s_wind_dir_gnd, -2, 2)
    G.s_wind_dir_8k = gen_rand_dev(G.s_wind_dir_gnd, -3, 3)
    G.s_wind_speed_2k = gen_rand_dev_speed(G.s_wind_speed_gnd, -2, 2)
    G.s_wind_speed_8k = gen_rand_dev_speed(G.s_wind_speed_gnd, -2, 2)
...
def get_mission_date_time():  # get the time date store in G.
    ...
    elif G.i_time_index == 101:  # this is for new array table to pic nights with full moon
        a = datetime.datetime.now()
        now = a + datetime.timedelta(0, 180)
        G.s_year = str(now.year)
        G.s_month = str(now.month)
        G.s_day = G.l_dates_new[now.month-1][0]  # subtract one index in array
        G.s_hour = G.l_dates_new[now.month-1][1]
        G.s_mins = '0'
        G.s_seconds = '0'
...
def is_sandstorm_terrain(l_theatre_id):
    if l_theatre_id[0] == 'PersianGulf' or l_theatre_id[0] == 'Syria':
        return True
    else:
        return False
...
def save_date_and_time(l_mission):  # save time/date info into mission list
    ...
    i_forced = find_item_index(l_mission, 'end of ["forcedOptions"]', i_mission_size)
...
def save_cloud_atmosphere(l_mission):  # save cloud data into mission file list
    ...
    if G.b_daws_mission:
        ...
        l_mission[i_start_clouds + 5] = '\t\t\t' + s_cloud_base + G.s_cloud_base_m + ","  # cloud base

        change_mission_data_item(l_mission, s_cloud_rain, G.s_iprecptns, '\t\t\t')
        #  l_mission[i_start_clouds + 5] = '\t\t\t' + s_cloud_rain + G.s_iprecptns + ","
    # new clouds preset handling code
    if not G.s_cloud_preset == 'none':
        l_mission[i_start_clouds + 4] = '\t\t\t' + s_cloud_preset + '"' + G.s_cloud_preset + '",'  # cloud preset
...
def print_all_data():  # debug just so we can see what values we have
    ...
    print("G.s_cloud_preset is ", G.s_cloud_preset)
...
if G.b_update_weather:
    if G.b_debug_load:
        ...
        if G.s_api_key_checkwx == 'NOT_YET_SET':
            ...
            print("Trying to use Primary METAR Server checkwx")
            get_weather_checkwx()
        ...
        if G.json_weather is False:
            if G.s_api_key_avwx == 'NOT_YET_SET':
                ...
                print("USING Secondary METAR Server avxw")
                get_weather_avwx()
